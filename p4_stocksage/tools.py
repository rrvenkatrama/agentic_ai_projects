...
def _load_earnings_to_chroma(ticker: str) -> chromadb.Collection:
    ...
    # Return existing collection if already loaded
    try:
    ...
    transcript_files = list(data_dir.glob(f"{ticker}_*.txt"))
...
def get_earnings_context(ticker: str, query: str, top_k: int = 3) -> dict:
    """
    Retrieve relevant earnings transcript chunks for a query.

    Args:
        ticker: Stock symbol (e.g., "AAPL")
        query: Question to answer (e.g., "What did management say about AI?")
        top_k: Number of chunks to return

    Returns:
        Dict with relevant context chunks and source info
    """
    try:
        collection = _load_earnings_to_chroma(ticker)
...
if __name__ == "__main__":
    ...
    if not NEWSAPI_KEY:
        ...
        print(json.dumps(get_news_sentiment("AAPL"), indent=2))

    
    print("EARNINGS RAG TEST")
    print("=" * 60)
    result = get_earnings_context("AAPL", "What did management say about AI?")
